...
def render(history_df, webhook_url):
    ...
    with col1:
        ...
        with c_s1: season_val = st.selectbox("이적 시즌 / 시장", ["26/27 여름 (Summer)", "26/27 겨울 (Winter)", "기타"], index=0, key=f"season_{k_id}")
        ...
        with c_n1: player_name = st.text_input("선수 이름", placeholder="예: Bruno Guimarães", key=f"name_{k_id}")
        with c_n2: player_nat = st.text_input("국적", placeholder="예: 브라질", key=f"nat_{k_id}")
        with c_n3: player_age = st.number_input("만 나이", min_value=15, max_value=45, value=28, key=f"age_{k_id}")

        c_t1, c_t2, c_t3 = st.columns(3)
        with c_t1: in_from_team = st.text_input("원소속팀명 (보내는 팀)", placeholder="예: 뉴캐슬", key=f"from_team_{k_id}")
        with c_t2: in_to_team = st.text_input("이적팀명 (영입 구단)", placeholder="예: 맨체스터 시티", key=f"to_team_{k_id}")
        with c_t3: in_to_league_choice = st.selectbox("이적팀 리그", list(LEAGUE_WEIGHTS.keys()), index=0, key=f"to_league_choice_{k_id}")
        
        pos_col1, pos_col2 = st.columns(2)
        with pos_col1: main_position = st.selectbox("주 포지션", list(POSITION_WEIGHTS.keys()), index=2, key=f"pos_{k_id}")
        ...
        st.markdown("---")
        tm_market_value = st.number_input("TM시장가치(만€)", min_value=0, value=8500, step=50, key=f"tm_{k_id}")
        actual_transfer_fee = st.number_input("실제이적료(만€)", min_value=0, value=10000, step=50, key=f"fee_{k_id}")
        weekly_wage_in = st.number_input("주급(만€)", min_value=0.0, value=30.0, step=0.5, key=f"wage_{k_id}")
        player_notes = st.text_area("스카우팅메모", placeholder="특이사항 입력", key=f"note_{k_id}")
    ...
    btn_label = f"🔄 '{player_name}' 구글 시트 업데이트(수정)" if edit_toggle else "💾 구글 시트에 신규 저장하기"
