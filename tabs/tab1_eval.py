...
def render(history_df, webhook_url):
            ...
            with c_ld3:
                ...
                if st.button("📥 데이터 불러오기", type="primary", use_container_width=True, key="tab1_load_btn"):
                    if sel_e_player:
                        ...
                        # 시트 데이터를 불러와서 dedicated 세션에 통째로 저장
                        p_pos_str = str(get_exact_val(row_raw, "포지션", ""))
                        pos_match = list(POSITION_WEIGHTS.keys())[2]
                        ...
                        st.session_state["last_saved_msg"] = f"✅ '{sel_e_player}' 데이터 불러오기 성공!"
                        st.rerun()
    else:
        st.session_state["edit_row_index"] = None
        if not edit_toggle and st.session_state.get("loaded_data"):
            st.session_state["loaded_data"] = {}

    ld = st.session_state.get("loaded_data", {})

    st.markdown("---")
    trade_type_choice = st.radio("거래 유형 구분", ["🔵 영입 (IN)", "🔴 방출 / 판매 (OUT)"], index=0, horizontal=True, key="tab1_trade_type")
    ...
    with col1:
        ...
        with c_s1: season_val = st.selectbox("이적 시즌 / 시장", ["26/27 여름 (Summer)", "26/27 겨울 (Winter)", "기타"], index=0, key="tab1_season")
        with c_s2: transfer_type = st.selectbox("이적 형태 & 계약 조항", list(TRANSFER_TYPE_WEIGHTS.keys()), index=list(TRANSFER_TYPE_WEIGHTS.keys()).index(ld.get("ttype")) if ld.get("ttype") in TRANSFER_TYPE_WEIGHTS else 0, key="tab1_ttype")
            
        option_exercised = st.checkbox("📌 임대 후 옵션 발동 (완전 전환 완료된 건)", value=False, key="tab1_opt")
        if option_exercised:
            transfer_type = "일반 완전 이적 (Permanent, 기준)"

        c_n1, c_n2, c_n3 = st.columns([2, 1, 1])
        with c_n1: player_name = st.text_input("선수 이름", value=ld.get("name", ""), placeholder="예: Bruno Guimarães", key="tab1_name")
        with c_n2: player_nat = st.text_input("국적", value=ld.get("nat", ""), placeholder="예: 브라질", key="tab1_nat")
        with c_n3: player_age = st.number_input("만 나이", min_value=15, max_value=45, value=ld.get("age", 28), key="tab1_age")

        c_t1, c_t2, c_t3 = st.columns(3)
        with c_t1: in_from_team = st.text_input("원소속팀명 (보내는 팀)", value=ld.get("from_team", ""), placeholder="예: 뉴캐슬", key="tab1_from_team")
        with c_t2: in_to_team = st.text_input("이적팀명 (영입 구단)", value=ld.get("to_team", ""), placeholder="예: 맨체스터 시티", key="tab1_to_team")
        
        to_league_keys = list(LEAGUE_WEIGHTS.keys())
        def_to_l_idx = to_league_keys.index(ld.get("to_league")) if ld.get("to_league") in to_league_keys else 0
        with c_t3: in_to_league_choice = st.selectbox("이적팀 리그", to_league_keys, index=def_to_l_idx, key="tab1_to_league")
        
        pos_keys = list(POSITION_WEIGHTS.keys())
        def_pos_idx = pos_keys.index(ld.get("pos")) if ld.get("pos") in pos_keys else 2
        pos_col1, pos_col2 = st.columns(2)
        with pos_col1: main_position = st.selectbox("주 포지션", pos_keys, index=def_pos_idx, key="tab1_pos")
        with pos_col2: versatility = st.selectbox("멀티 포지션 소화 능력", list(VERSATILITY_WEIGHTS.keys()), index=0, key="tab1_vers")
            
        c_r1, c_r2 = st.columns(2)
        with c_r1: reg_status = st.selectbox("스쿼드 등록 / HG 쿼터", list(REGISTRATION_WEIGHTS.keys()), index=0, key="tab1_reg")
        with c_r2: big_stage = st.selectbox("UCL / 빅매치 검증도", list(BIG_STAGE_WEIGHTS.keys()), index=0, key="tab1_stage")
        
        c_i1, c_i2 = st.columns(2)
        with c_i1: injury_status = st.selectbox("부상 내구성 & 메디컬 리스크", list(INJURY_WEIGHTS.keys()), index=1, key="tab1_inj")
        with c_i2: urgency_status = st.selectbox("영입 구단 절박성 & 취약 포지션", list(URGENCY_WEIGHTS.keys()), index=0, key="tab1_urg")

        def_from_l_idx = to_league_keys.index(ld.get("from_league")) if ld.get("from_league") in to_league_keys else 0
        selling_league = st.selectbox("보내는 리그 (원소속 리그)", to_league_keys, index=def_from_l_idx, key="tab1_from_league")
        
        tier_keys = list(CLUB_TIERS.keys())
        def_tier_idx = tier_keys.index(ld.get("tier")) if ld.get("tier") in tier_keys else 1
        buying_club_tier = st.selectbox("영입구단티어", tier_keys, index=def_tier_idx, key="tab1_tier")
        
        remaining_contract = st.selectbox("이적 당시 잔여 계약 기간", list(CONTRACT_WEIGHTS.keys()), index=2, key="tab1_contract")
        
        st.markdown("---")
        tm_market_value = st.number_input("TM시장가치(만€)", min_value=0, value=ld.get("tm", 8500), step=50, key="tab1_tm")
        actual_transfer_fee = st.number_input("실제이적료(만€)", min_value=0, value=ld.get("fee", 10000), step=50, key="tab1_fee")
        weekly_wage_in = st.number_input("주급(만€)", min_value=0.0, value=ld.get("wage", 30.0), step=0.5, key="tab1_wage")
        player_notes = st.text_area("스카우팅메모", value=ld.get("notes", ""), placeholder="특이사항 입력", key="tab1_notes")
    ...
    btn_label = f"🔄 '{player_name or '선수'}' 구글 시트 업데이트(수정)" if edit_toggle else "💾 구글 시트에 신규 저장하기"

    if st.button(btn_label, type="primary", use_container_width=True, key="tab1_save_btn"):
