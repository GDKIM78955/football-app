...
def render(history_df, webhook_url):
            ...
            with c_ld3:
                ...
                if st.button("📥 데이터 불러오기", type="primary", use_container_width=True, key="tab1_load_btn"):
                    if sel_e_player:
                        ...
                        # 🌟 불러온 값을 세션 위젯 키에 직접 할당하여 화면에 즉시 반영
                        st.session_state["tab1_name"] = str(get_exact_val(row_raw, "선수명", ""))
                        st.session_state["tab1_nat"] = str(get_exact_val(row_raw, "국적", ""))
                        st.session_state["tab1_age"] = int(get_exact_val(row_raw, "만나이", 28))
                        st.session_state["tab1_from_team"] = str(get_exact_val(row_raw, "원소속팀명", ""))
                        st.session_state["tab1_to_team"] = str(get_exact_val(row_raw, "이적팀명", ""))
                        st.session_state["tab1_tm"] = int(get_exact_val(row_raw, "TM시장가치(만€)", 4500))
                        st.session_state["tab1_fee"] = int(get_exact_val(row_raw, "실제이적료(만€)", 0))
                        st.session_state["tab1_wage"] = float(get_exact_val(row_raw, "주급(만€)", 0.0))
                        
                        p_notes = str(get_exact_val(row_raw, "스카우팅메모", ""))
                        st.session_state["tab1_notes"] = p_notes.split(" | [영입")[0].split(" | [방출")[0].strip()

                        # 셀렉트박스 값 매칭
                        p_pos_str = str(get_exact_val(row_raw, "포지션", ""))
                        for p_k in POSITION_WEIGHTS.keys():
                            if p_pos_str and p_pos_str in p_k:
                                st.session_state["tab1_pos"] = p_k
                                break

                        p_from_league = str(get_exact_val(row_raw, "원소속리그", ""))
                        for l_k in LEAGUE_WEIGHTS.keys():
                            if p_from_league and p_from_league in l_k:
                                st.session_state["tab1_from_league"] = l_k
                                break

                        p_to_league_name = str(get_exact_val(row_raw, "이적팀리그", ""))
                        for l_k in LEAGUE_WEIGHTS.keys():
                            if p_to_league_name and p_to_league_name in l_k:
                                st.session_state["tab1_to_league"] = l_k
                                break

                        p_tier = str(get_exact_val(row_raw, "영입구단티어", ""))
                        for t_k in CLUB_TIERS.keys():
                            if p_tier and p_tier in t_k:
                                st.session_state["tab1_tier"] = t_k
                                break

                        p_ttype = str(get_exact_val(row_raw, "이적형태", ""))
                        for tt_k in TRANSFER_TYPE_WEIGHTS.keys():
                            if p_ttype and p_ttype in tt_k:
                                st.session_state["tab1_ttype"] = tt_k
                                break

                        st.session_state["last_saved_msg"] = f"✅ '{sel_e_player}' 데이터 불러오기 완료!"
                        st.rerun()
    else:
    ...
    with col1:
        ...
        with c_s2: transfer_type = st.selectbox("이적 형태 & 계약 조항", list(TRANSFER_TYPE_WEIGHTS.keys()), index=0, key="tab1_ttype")
        ...
        with c_n1: player_name = st.text_input("선수 이름", placeholder="예: Bruno Guimarães", key="tab1_name")
        with c_n2: player_nat = st.text_input("국적", placeholder="예: 브라질", key="tab1_nat")
        with c_n3: player_age = st.number_input("만 나이", min_value=15, max_value=45, value=28, key="tab1_age")

        c_t1, c_t2, c_t3 = st.columns(3)
        with c_t1: in_from_team = st.text_input("원소속팀명 (보내는 팀)", placeholder="예: 뉴캐슬", key="tab1_from_team")
        with c_t2: in_to_team = st.text_input("이적팀명 (영입 구단)", placeholder="예: 맨체스터 시티", key="tab1_to_team")
        with c_t3: in_to_league_choice = st.selectbox("이적팀 리그", list(LEAGUE_WEIGHTS.keys()), index=0, key="tab1_to_league")
        
        pos_keys = list(POSITION_WEIGHTS.keys())
        pos_col1, pos_col2 = st.columns(2)
        with pos_col1: main_position = st.selectbox("주 포지션", pos_keys, index=2, key="tab1_pos")
        ...
        selling_league = st.selectbox("보내는 리그 (원소속 리그)", list(LEAGUE_WEIGHTS.keys()), index=0, key="tab1_from_league")
        buying_club_tier = st.selectbox("영입구단티어", list(CLUB_TIERS.keys()), index=1, key="tab1_tier")
        remaining_contract = st.selectbox("이적 당시 잔여 계약 기간", list(CONTRACT_WEIGHTS.keys()), index=2, key="tab1_contract")
        
        st.markdown("---")
        tm_market_value = st.number_input("TM시장가치(만€)", min_value=0, value=8500, step=50, key="tab1_tm")
        actual_transfer_fee = st.number_input("실제이적료(만€)", min_value=0, value=10000, step=50, key="tab1_fee")
        weekly_wage_in = st.number_input("주급(만€)", min_value=0.0, value=30.0, step=0.5, key="tab1_wage")
        player_notes = st.text_area("스카우팅메모", placeholder="특이사항 입력", key="tab1_notes")
