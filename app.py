    ...
    with st.expander(
      ...
      if "GK" in main_position:
        st.markdown(f"""
                ...
                - **득점 차단 (선방력)**: `{st.session_state.get('f_gk_prevented', 2.45):+.2f shame}` (출전 {st.session_state['f_mins']:,}분)
...
# ================= TAB 6: 이적시장 구단/리그별 종합 결산 & 데이터룸 (양방향 집계 완벽 수정) =================
with tab6:
  st.subheader(
  ...
  if (
    ...
    if "구단별" in rank_mode:
      ...
      if not all_club_names:
        ...
        # 🌟 [수정] 거래구분 태그와 무관하게 '이적팀명 == 구단'이면 무조건 영입(IN)으로 완벽 집계
        team_in_df = (
            club_filtered_df[
        ...
        # 🌟 [수정] 거래구분 태그와 무관하게 '원소속팀명 == 구단'이면 무조건 방출(OUT)으로 완벽 집계
        team_out_df = (
            club_filtered_df[
      ...
      if not auto_detected_leagues:
        ...
        if l_target_df.empty:
          ...
          def calc_team_stats(group):
            t_name = group["이적팀명"].iloc[0]
            in_trades = group[group["이적팀명"] == t_name]
            out_trades = (
                league_filtered_df[league_filtered_df["원소속팀명"] == t_name]
          ...
          team_group = (
              l_target_df.groupby("이적팀명")
              .apply(calc_team_stats)
              .reset_index()
          )

          ranked_df = team_group.sort_values(
