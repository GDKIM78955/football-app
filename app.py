...
with tab1:
            ...
            with c_ld3:
                ...
                if st.button("📥 데이터 불러오기", type="primary", use_container_width=True):
                    if sel_e_player:
                        ...
                        # 🌟 [핵심 해결] 폼 키 ID를 갱신하여 기존 위젯 메모리를 초기화하고 시트 값을 강제로 주입
    ...
    with col1:
        ...
        with c_n1: player_name = st.text_input("선수 이름", placeholder="예: Ezri Konsa", value=st.session_state.get(f"name_{k_id}", ""), key=f"name_{k_id}")
        with c_n2: player_nat = st.text_input("국적", placeholder="예: 잉글랜드", value=st.session_state.get(f"nat_{k_id}", ""), key=f"nat_{k_id}")
        ...
        with c_t1: in_from_team = st.text_input("원소속팀명 (보내는 팀)", placeholder="예: 아스톤 빌라", value=st.session_state.get(f"from_team_{k_id}", ""), key=f"from_team_{k_id}")
        with c_t2: in_to_team = st.text_input("이적팀명 (영입 구단)", placeholder="예: 아스날", value=st.session_state.get(f"to_team_{k_id}", ""), key=f"to_team_{k_id}")
        ...
        actual_transfer_fee = st.number_input(
            fee_label, 
            min_value=0, 
            value=0 if is_undisclosed else st.session_state.get(f"fee_{k_id}", 0), 
            step=50, 
            key=f"fee_{k_id}",
        ...
        player_notes = st.text_area("스카우팅메모", placeholder="예: 대인 방어 및 후방 빌드업 우수", value=st.session_state.get(f"note_{k_id}", ""), key=f"note_{k_id}")
