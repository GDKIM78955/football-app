...
with tab1:
  ...
  with col1:
    st.subheader(f"📝 {'방출(OUT)' if is_out_trade else '영입(IN)'} 선수 & 계약 정보")
    ...
    with c_s2:
      transfer_type = st.selectbox(
          "이적 형태 & 계약 조항",
          list(TRANSFER_TYPE_WEIGHTS.keys()),
          index=0,
          key=f"ttype_{k_id}",
      )
    ...
    with c_n1:
      player_name = st.text_input(
          "선수 이름", value="", placeholder="예: Ezri Konsa", key=f"name_{k_id}"
      )
    with c_n2:
      player_nat = st.text_input(
          "국적", value="", placeholder="예: 잉글랜드", key=f"nat_{k_id}"
      )
    with c_n3:
      player_age = st.number_input(
          "나이(만)", min_value=15, max_value=45, value=28, key=f"age_{k_id}"
      )

    if (
        player_name.strip()
        and not history_df.empty
        and "선수명" in history_df.columns
    ):
      dup_matches = history_df[
          (
              history_df["선수명"].astype(str).str.strip().str.lower()
      ...
      if not dup_matches.empty:
        ...
        st.warning(
            f"⚠️ **중복 등록 알림**: **'{player_name.strip()}'** 선수는 이미 이번"
            f" `{season_val}` 시즌에 등록된 내역이 있습니다! (`[{dup_from} ➔"
            f" {dup_to}] | €{dup_fee:,.0f}만`)"
        )

    c_t1, c_t2, c_t3 = st.columns(3)
    with c_t1:
      in_from_team = st.text_input(
          "원소속팀명 (보내는 팀)",
          value="",
          placeholder="예: 아스톤 빌라",
          key=f"from_team_{k_id}",
      )
    with c_t2:
      in_to_team = st.text_input(
          "이적팀명 (영입 구단)",
          value="",
          placeholder="예: 아스날",
          key=f"to_team_{k_id}",
    ...
    with c_t3:
      in_to_league_choice = st.selectbox(
          "이적팀 리그",
          list(LEAGUE_WEIGHTS.keys()),
          index=0,
          key=f"to_league_choice_{k_id}",
      )

    is_tracked_target = any(
        k in in_to_league_choice for k in TRACKED_LEAGUE_NAMES
    )
    if is_tracked_target:
      st.caption(
          "✅ **10대 핵심 리그 이적**: 시즌 종료 후 4번 탭 사후 검증 대상에"
          " **자동 등록**됩니다."
      )
    else:
      st.caption(
          "ℹ️ **기타 리그 이적**: 메인 결산 장부에만 기록되며, 검증 시트에는"
          " 등록되지 않습니다."
      )
    ...
    with pos_col2:
      versatility = st.selectbox(
          "멀티 포지션 소화 능력",
          list(VERSATILITY_WEIGHTS.keys()),
          index=0,
          key=f"vers_{k_id}",
      )
    ...
    with c_r1:
      reg_status = st.selectbox(
          "스쿼드 등록 / HG 쿼터",
          list(REGISTRATION_WEIGHTS.keys()),
          index=1,
          key=f"reg_{k_id}",
      )
    with c_r2:
      big_stage = st.selectbox(
          "UCL / 빅매치 검증도",
          list(BIG_STAGE_WEIGHTS.keys()),
          index=0,
          key=f"stage_{k_id}",
      )
    ...
    with c_i1:
      injury_status = st.selectbox(
          "부상 내구성 & 메디컬 리스크",
          list(INJURY_WEIGHTS.keys()),
          index=1,
          key=f"inj_{k_id}",
      )
    with c_i2:
      urgency_status = st.selectbox(
          "영입 구단 절박성 & 취약 포지션",
    ...
    selling_league = st.selectbox(
        "보내는 리그 (원소속 리그)",
        list(LEAGUE_WEIGHTS.keys()),
        index=0,
        key=f"league_{k_id}",
    )
    buying_club_tier = st.selectbox(
        "영입하는 구단 규모",
        list(CLUB_TIERS.keys()),
        index=1,
        key=f"tier_{k_id}",
    )
    remaining_contract = st.selectbox(
        "이적 당시 잔여 계약 기간",
        list(CONTRACT_WEIGHTS.keys()),
        index=2,
        key=f"contract_{k_id}",
    )

    st.markdown("---")

    f_p90 = (
        (st.session_state["f_mins"] / 90.0)
        if st.session_state["f_mins"] > 0
        else 1.0
    )
    ...
    with st.expander(
        "🔗 [FotMob 탭 연동] 지난 시즌 실적 및 평점 가중치", expanded=True
    ):
    ...
    with st.expander(
        "📚 [클릭하여 확인] 12대 가중치 전체 세부 산정 기준표 & 가이드",
        expanded=False,
    ):
    ...
    fee_label = (
        "실제 수령/지출 임대료 (Loan Fee, 만 유로, €)"
        if is_loan_type
        else (
            "실제 방출(판매) 이적료 (만 유로, €)"
            if is_out_trade
            else "실제 영입(지출) 이적료 (만 유로, €)"
        )
    )
    ...
    actual_transfer_fee = st.number_input(
        fee_label,
        min_value=0,
        value=0 if is_undisclosed else (5960 if not is_loan_type else 500),
        step=50,
        key=f"fee_{k_id}",
    ...
    if is_undisclosed:
      st.info(
          "💡 **비공개 이적 선택됨**: 실제 이적료가 공개되지 않아 시장 적정가와"
          " 동일하게 추정하여 분석합니다."
      )
    elif actual_transfer_fee > 0:
      st.caption(
          f"💡 실제금액 환산: **{format_currency_desc(actual_transfer_fee)}**"
      )

    with st.expander(
        "💼 [선택 입력] 주급(Weekly Wage) & 연간 총비용(Total Package) 분석",
        expanded=False,
    ):
      weekly_wage_in = st.number_input(
          "선수 주급 (만 유로, €/주)",
          min_value=0.0,
          value=0.0,
          step=0.5,
      ...
      annual_transfer_amort = (
          (actual_transfer_fee / 4.0) if actual_transfer_fee > 0 else 0.0
      )
      total_annual_cost = annual_transfer_amort + annual_wage_eur
      if weekly_wage_in > 0:
        st.caption(
            f"📌 **주급 환산**: 주당 약"
            f" {weekly_wage_in*10000*rate_krw/100000000:.1f}억원"
            f" (£{weekly_wage_in*rate_gbp:.1f}만)"
        )
        st.markdown(
            f"- **연간 총비용 (이적료 4년 분할상각 + 1년 연봉)**:"
            f" `€{total_annual_cost:,.1f}만` (약"
            f" {total_annual_cost*10000*rate_krw/100000000:.0f}억원/년)"
        )
      else:
        st.caption(
            "ℹ️ 주급이 입력되지 않았습니다. (주급 미반영 순수 이적료"
            " 기준으로 분석)"
        )

    player_notes = st.text_area(
        "개인 메모 / 스카우팅 코멘트",
        placeholder="예: 대인 방어 및 후방 빌드업 우수",
        key=f"note_{k_id}",
    )
  ...
  if is_undisclosed:
    ...
    status_label = (
        f"⚠️ {'고가 매각 성공' if is_out_trade else '고평가/오버페이'}"
        f" (+{overpay_pct:.1f}%)"
    )
  else:
    status_label = (
        f"💎 {'헐값 매각 손해' if is_out_trade else '저평가/혜자'}"
        f" ({overpay_pct:.1f}%)"
    )
  ...
  if tm_market_value > 0 and (
      calc_actual_fee > 0
      or is_loan_type
      or "FA" in transfer_type
      or is_undisclosed
  ):
    ...
    val_score_delta = (
        0.0
        if is_undisclosed
        else max(-3.5, min(2.5, score_multiplier * (overpay_pct / 20.0)))
    )
    ...
    ext_val_score_delta = (
        0.0
        if is_undisclosed
        else max(-3.5, min(2.5, score_multiplier * (ext_overpay_pct / 20.0)))
    )
    ext_deal_score = round(
        max(
            1.00,
            min(
                10.00,
    ...
    def get_grade_info(score):
      if score >= 9.00:
        return "💎 S등급 (Masterclass Deal)", "success"
      elif score >= 8.00:
        return "🌟 A등급 (Excellent Deal)", "success"
      elif score >= 7.00:
        return "⚖️ B등급 (Solid / Fair Deal)", "info"
      elif score >= 6.00:
        return "⚠️ C등급 (Risky Deal)", "warning"
      else:
        return "🚨 D등급 (Panic / Bad Deal)", "error"
  ...
  with col2:
    ...
    transfer_route = (
        f"[{in_from_team} ➔ {in_to_team}]"
        if in_from_team.strip() and in_to_team.strip()
        else ""
    )
    tag_badge = "🔴 [방출/판매]" if is_out_trade else "🔵 [영입/보강]"
    st.markdown(
        f"### {tag_badge} **{display_name}** {display_nat} {transfer_route} -"
        f" `{pos_short}` {season_icon}"
    )
    st.caption(
        f"📌 시장: **{season_val.split(' (')[0]}** | 형태: **{ttype_short}** |"
        f" 쿼터: **{reg_short}** | 필요도: **{urg_short}**"
    )
    ...
    with res_c1:
      ...
      if fair_value > 0:
        st.caption(f"{format_currency_desc(fair_value).split(' | ')[0]}")
    with res_c2:
      fee_display = (
          "비공개 (추정)" if is_undisclosed else f"€{calc_actual_fee:,.1f}만"
      )
      st.metric(
          "실제 거래액",
          fee_display,
          delta=(
              f"{diff:+,.1f}만 (€)"
              if not is_undisclosed and calc_actual_fee > 0
              else None
          ),
          delta_color="inverse" if not is_out_trade else "normal",
      )
      if not is_undisclosed and calc_actual_fee > 0:
        st.caption(
            f"{format_currency_desc(calc_actual_fee).split(' | ')[0]}"
        )
    with res_c3:
      st.metric(
          "평가율 / 진단",
          (
              f"{overpay_pct:+.1f}%"
              if fair_value > 0 and not is_undisclosed
              else "0.0%"
          ),
          delta=status_label.split(" ")[0],
      )
      st.caption(status_label)
    with res_c4:
      st.metric(
          "이적 거래 평점",
          f"★ {final_deal_score:.2f}",
          delta=deal_grade.split(" ")[0],
      )
    ...
    with st.expander(
        "📊 [이미지 캡처용] 선수 12대 스카우팅 육각형 레이더 차트", expanded=True
    ):
      radar_categories = [
          "리그 템포",
          "나이/포텐",
      ...
      fig.add_trace(
          go.Scatterpolar(
              ...
              line=dict(
                  color="#1f77b4" if not is_out_trade else "#d62728", width=2
              ),
              name=display_name,
          )
      )
    ...
    st.markdown(
        "#### 📢 **[외부 발표용] 시장가 범위 & 진단 평점"
        f" ({season_icon} {season_val.split(' ')[1] if ' ' in season_val else ''})**"
    )
    ...
    if fair_value > 0:
      ...
      with ext_c2:
        fee_ext_str = (
            "비공개 (추정)" if is_undisclosed else f"€{calc_actual_fee:,.0f}만"
        )
        st.markdown(f"""
    ...
    with st.expander(
        "🔍 [실시간 확인] 12대 세부 가중치 적용 현황표 & 누적 배율",
        expanded=True,
    ):
      total_multiplier = (
          league_w
    ...
    if player_name.strip() and (
        tm_market_value > 0
        or calc_actual_fee > 0
        or is_loan_type
        or "FA" in transfer_type
        or is_undisclosed
    ):
      with st.expander(
          "📋 [클릭하여 복사] 외부 발표용 공식 브리핑 요약 텍스트",
          expanded=True,
      ):
        nat_text = (
            f"({player_nat}, 만 {player_age}세)"
            if player_nat
            else f"(만 {player_age}세)"
        )
        route_text = (
            f"[{in_from_team} ➔ {in_to_team}]"
            if in_from_team.strip() and in_to_team.strip()
        ...
        fee_brf_str = (
            ...
            if is_undisclosed
                ...
                f"€{calc_actual_fee:,.1f}만"
                f" ({format_currency_desc(calc_actual_fee)})"
            )
        )

        summary_text = f"""⚽ [{season_val} {tag_badge} 공식 이적 분석 브리핑] {player_name} {nat_text} {route_text}
━━━━━━━━━━━━━━━━━━━━
...
"""
        if player_notes.strip():
          summary_text += f"📝 스카우팅 메모: {player_notes.strip()}\n"
        st.code(summary_text.strip(), language="text")

# ================= TAB 2: FotMob 시즌 성적 & 이적 예측 =================
with tab2:
  st.subheader(
      "📱 FotMob 스타일 시즌 스탯 입력 & 이적 첫 시즌 성적 프로젝션"
  )
  ...
  with f_c1:
    f_pos = st.selectbox(
        "선수 포지션 분류",
        [
            "⚽ 필드 플레이어 (공격수/미드필더/수비수)",
            "🧤 골키퍼 (Goalkeeper)",
        ],
        index=1 if "GK" in main_position else 0,
        key="f_tab_pos",
    )
  with f_c2:
    f_from_l = st.selectbox(
        "원소속 리그 (기록 기준)",
        list(LEAGUE_WEIGHTS.keys()),
        index=0,
        key="f_tab_from_l",
  ...
  with f_c3:
    f_to_l = st.selectbox(
        ...
        list(LEAGUE_WEIGHTS.keys()),
        index=(
            list(LEAGUE_WEIGHTS.keys()).index(in_to_league_choice)
            if in_to_league_choice in LEAGUE_WEIGHTS
            else 0
        ),
        key="f_tab_to_l",
    )
  ...
  st.divider()
  st.markdown(
      "### 📥 FotMob 시즌 실제 기록 입력 (지난 시즌/전반기 스탯)"
  )

  b1, b2, b3, b4 = st.columns(4)
  with b1:
    in_matches = st.number_input(
        "출전 경기 (Matches)",
        1,
        60,
        value=min(int(st.session_state["f_matches"]), 60),
        key="in_matches_box",
    )
  with b2:
    in_starts = st.number_input(
        "선발 출전 (Starts)",
        0,
        60,
        value=min(int(st.session_state["f_starts"]), 60),
        key="in_starts_box",
    )
  with b3:
    in_mins = st.number_input(
        "출전 시간 (Minutes)",
        90,
        4500,
        value=min(int(st.session_state["f_mins"]), 4500),
        key="in_mins_box",
    )
  with b4:
    in_rating = st.number_input(
        "FotMob 평균 평점",
        5.0,
        10.0,
        value=float(st.session_state["f_rating"]),
        step=0.01,
        key="in_rating_box",
    )
  ...
  if "골키퍼" not in f_pos:
    ...
    with s1:
      in_goals = st.number_input(
          "득점 (Goals)",
          0,
          50,
          value=min(int(st.session_state["f_goals"]), 50),
          key="in_goals_box",
      )
    with s2:
      in_xg = st.number_input(
          "기대 득점 (xG)",
          0.0,
          50.0,
          value=min(float(st.session_state["f_xg"]), 50.0),
          step=0.01,
          key="in_xg_box",
      )
    with s3:
      in_shots = st.number_input(
          "총 슈팅 (Shots)",
          0,
          200,
          value=min(int(st.session_state["f_shots"]), 200),
          key="in_shots_box",
      )
    with s4:
      in_sot = st.number_input(
          "유효 슈팅 (On Target)",
          0,
          100,
          value=min(int(st.session_state["f_sot"]), 100),
          key="in_sot_box",
      )
    with s5:
      in_pk_goals = st.number_input(
          "PK 득점 (Penalty)", 0, 20, 0, key="in_pk_box"
      )
    ...
    with p1:
      in_assists = st.number_input(
          "도움 (Assists)",
          0,
          50,
          value=min(int(st.session_state["f_assists"]), 50),
          key="in_assists_box",
      )
    with p2:
      in_xa = st.number_input(
          "기대 도움 (xA)",
          0.0,
          50.0,
          value=min(float(st.session_state["f_xa"]), 50.0),
          step=0.01,
          key="in_xa_box",
      )
    with p3:
      in_chances = st.number_input(
          "기회 창출 (Chances)",
          0,
          150,
          value=min(int(st.session_state["f_chances"]), 150),
          key="in_chances_box",
      )
    with p4:
      in_big_chances = st.number_input(
          "빅 찬스 메이킹", 0, 50, 2, key="in_bc_box"
      )
    with p5:
      in_pass_pct = st.number_input(
          "패스 성공률 (%)", 30.0, 100.0, 88.2, 0.1, key="in_pass_pct_box"
      )
    ...
    st.markdown(
        "#### 3️⃣ 경합 및 수비 기여 (Duels & Defending - 공격/수비"
        " 전원 기록)"
    )
    d1, d2, d3, d4, d5 = st.columns(5)
    with d1:
      in_dribbles = st.number_input(
          "성공한 드리블",
          0,
          100,
          value=min(int(st.session_state["f_dribbles"]), 100),
          key="in_dribbles_box",
      )
    with d2:
      in_touches_box = st.number_input(
          "박스 안 터치 (Box Touches)",
          0,
          300,
          value=min(int(st.session_state["f_touches_box"]), 300),
          key="in_touches_box",
      )
    with d3:
      in_duels_pct = st.number_input(
          "지상 경합 승률 (%)", 20.0, 100.0, 62.4, 0.1, key="in_duels_box"
      )
    with d4:
      in_aerial_pct = st.number_input(
          "공중볼 승률 (%)", 20.0, 100.0, 65.8, 0.1, key="in_aerial_box"
      )
    with d5:
      in_tackles = st.number_input(
          "태클 성공 (Tackles)",
          0,
          150,
          value=min(int(st.session_state["f_tackles"]), 150),
          key="in_tackles_box",
      )
    ...
    with gk1:
      in_gk_saves = st.number_input(
          "선방 (Saves)",
          0,
          250,
          value=int(st.session_state["f_gk_saves"]),
          key="in_gk_saves_box",
      )
    with gk2:
      in_gk_conceded = st.number_input(
          "실점 수 (Goals Conceded)",
          0,
          120,
          value=int(st.session_state["f_gk_conceded"]),
          key="in_gk_conceded_box",
      )
    with gk3:
      in_gk_prevented = st.number_input(
          "득점 차단 (Goals Prevented)",
          -20.0,
          30.0,
          value=float(st.session_state["f_gk_prevented"]),
          step=0.01,
          key="in_gk_prevented_box",
      )

    gk4, gk5, gk6 = st.columns(3)
    with gk4:
      in_gk_cs = st.number_input(
          "클린 시트 (Clean Sheets)",
          0,
          35,
          value=int(st.session_state["f_gk_cs"]),
          key="in_gk_cs_box",
      )
    with gk5:
      in_gk_errors = st.number_input(
          "골로 이어진 실수 (Errors Led to Goal)",
          0,
          15,
          value=int(st.session_state["f_gk_errors"]),
          key="in_gk_errors_box",
      )
    with gk6:
      in_gk_claims = st.number_input(
          "공중에서 잡기 (High Claims)",
          0,
          80,
          value=int(st.session_state["f_gk_claims"]),
          key="in_gk_claims_box",
      )
  ...
  if "골키퍼" not in f_pos:
    ...
    st.markdown(
        "### 🎯 **FotMob 스타일 이적 첫 시즌 성적 예측 리포트"
        " (필드 플레이어)**"
    )
    st.caption(
        f"이적 환경: **{f_from_l.split(' ')[1]}** ➔ **{f_to_l.split(' ')[1]}**"
        f" | 최종 환산 계수: **{final_l_factor:.2f}x** ({adapt_desc})"
    )
    ...
    m1.metric(
        "예상 평점 (Rating)",
        f"★ {proj_rating:.2f}",
        delta=f"{proj_rating - in_rating:+.2f}",
    )
    m2.metric("예상 득점 (xG)", f"{proj_goals:.0f} 골", delta=f"xG {proj_xg:.2f}")
    m3.metric(
        "예상 도움 (xA)", f"{proj_assists:.0f} 도움", delta=f"xA {proj_xa:.2f}"
    )
    m4.metric(
        "예상 공격포인트",
        f"{proj_goals + proj_assists:.0f} P",
        delta=f"{proj_goals:.0f}G+{proj_assists:.0f}A",
    )
    m5.metric(
        "예상 슈팅 (유효)",
        f"{int(proj_shots)} 회",
        delta=f"유효 {int(proj_sot)}회",
    )

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        "#### 📊 **직전 시즌 실제 기록 vs 이적 첫 시즌 예측 데이터"
        " 비교표**"
    )
    ...
    proj_gk_saves = round(
        (in_gk_saves / base_p90) * target_p90 * (1.0 / raw_l_factor), 0
    )
    proj_gk_conceded = round(
        (in_gk_conceded / base_p90) * target_p90 * (1.0 / final_l_factor), 0
    )
    proj_gk_prevented = round(
        (in_gk_prevented / base_p90) * target_p90 * final_l_factor, 2
    )
    proj_gk_cs = round(
        (in_gk_cs / base_p90) * target_p90 * final_l_factor, 0
    )
    ...
    st.markdown(
        "### 🧤 **FotMob 스타일 이적 첫 시즌 골키퍼 성적 예측"
        " 리포트**"
    )
    st.caption(
        f"이적 환경: **{f_from_l.split(' ')[1]}** ➔ **{f_to_l.split(' ')[1]}**"
        f" | 리그 난이도 계수: **{final_l_factor:.2f}x** ({adapt_desc})"
    )
    ...
    gk_m2.metric(
        "예상 클린 시트",
        f"{int(proj_gk_cs)} 경기",
        delta=f"{int(proj_gk_cs - in_gk_cs):+d}회",
    )
    gk_m3.metric(
        "예상 득점 차단", f"{proj_gk_prevented:+.2f}", delta="선방력 지표"
    )
    gk_m4.metric(
        "예상 선방 횟수",
        f"{int(proj_gk_saves)} 회",
        delta=f"{int(proj_gk_saves - in_gk_saves):+d}회",
    )
    gk_m5.metric(
        "예상 실점 수",
        f"{int(proj_gk_conceded)} 실점",
        delta=f"{int(proj_gk_conceded - in_gk_conceded):+d}점",
        delta_color="inverse",
    )

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        "#### 📊 **FotMob 실제 지표 기준: 직전 시즌 vs 이적 첫 시즌 예측"
        " 비교표**"
    )
  ...
  display_pname = (
      player_name.strip() if player_name.strip() else "선수명 미입력"
  )
  tag_btn_name = "🔴 방출(OUT) 데이터" if is_out_trade else "🔵 영입(IN) 데이터"
  st.markdown(
      f"#### 💾 **'{display_pname}'** 선수의 {tag_btn_name} 구글 시트 저장"
  )

  if st.button(
      f"💾 {tag_btn_name} 구글 시트에 저장하기 (42개 항목 동기화)",
      type="primary",
    ...
    if not player_name.strip():
      ...
      with st.spinner("구글 시트에 거래 데이터를 기록 중입니다..."):
        ...
        detailed_notes = (
            f"[{'방출' if is_out_trade else '영입'}|{ttype_short}|{reg_short}|{urg_short}|UCL:{stage_w:.2f}|메디컬:{inj_w:.2f}]"
            f" 계약:{contract_desc}"
        )
        if player_notes.strip():
        ...
        if "골키퍼" in f_pos:
          detailed_notes += (
              f" |"
              f" GK[선방:{in_gk_saves}|실점:{in_gk_conceded}|차단:{in_gk_prevented:+.2f}|CS:{in_gk_cs}]"
          )
        ...
        payload = {
            ...
            "weekly_wage": (
                float(weekly_wage_in) if "weekly_wage_in" in locals() else 0.0
            ),
        }
        ...
        try:
          ...
          if res.status_code in [200, 302]:
            st.session_state["last_saved_msg"] = (
                f"✅ '{player_name}' 선수의 {tag_btn_name}가 성공적으로"
                " 저장되었습니다!"
            )
            st.cache_data.clear()
...
with tab3:
  st.subheader(
      "🔍 과거 유사 이적 사례 검색 및 벤치마크 비교 (Comps TOP 5 & 10)"
  )
  st.caption(
      "구글 시트에 누적된 이전 이적 데이터 중 이적료, 총 평점, 평가율(고평가/저평가),"
      " 출발 리그가 가장 유사한 과거 사례를 매칭합니다."
  )
  ...
  with c_in1:
    target_fee = st.number_input(
        "비교 기준 이적료 (만 €)",
        min_value=0,
        value=int(calc_actual_fee) if calc_actual_fee > 0 else 5000,
        step=100,
        key="comps_fee",
    )
  with c_in2:
    target_score = st.number_input(
        ...
        min_value=1.00,
        max_value=10.00,
        value=float(final_deal_score) if final_deal_score > 0 else 7.50,
        step=0.1,
        key="comps_score",
    )
  with c_in3:
    target_overpay = st.number_input(
        "비교 기준 평가율 (%)",
        min_value=-100.0,
        max_value=200.0,
        value=float(overpay_pct),
        step=1.0,
        key="comps_overpay",
    )
  with c_in4:
    pos_filter = st.selectbox(
        ...
        index=0,
        key="comps_pos_filter",
    )
  with c_in5:
    league_filter = st.selectbox(
        "원소속 리그 필터",
        ["전체 리그"] + list(LEAGUE_WEIGHTS.keys()),
        index=0,
        key="comps_league_filter",
    )
  ...
  if (
      history_df.empty
      or len(history_df) == 0
      or "선수명" not in history_df.columns
  ):
    st.info(
        "💡 **아직 구글 시트에 누적된 과거 이적 데이터가 없습니다.**\n\n1번 및"
        " 2번 탭에서 선수 데이터를 저장해 나가시면, 자동으로 이곳에 가장 유사한"
        " 과거 이적 사례 TOP 5 상세 카드 및 TOP 10 전체 목록이 나타나게 됩니다."
    )
  else:
    try:
      valid_rows = []
      for idx, row in history_df.iterrows():
        try:
          ...
          p_overpay = (
              ((p_fee - p_fair) / p_fair * 100) if p_fair > 0 else 0.0
          )
          ...
          fee_diff_norm = abs(p_fee - target_fee) / (
              max(target_fee, 1000) * 1.5
          )
          ...
          total_dist = (
              (fee_diff_norm * 0.30)
              + (score_diff_norm * 0.25)
              + (overpay_diff_norm * 0.25)
              + (league_diff_norm * 0.20)
          )
      ...
      if len(valid_rows) > 0:
        ...
        for i in range(0, len(top5_df), 2):
          ...
          for j in range(2):
            ...
            if idx_card < len(top5_df):
              ...
              with cols[j]:
                st.markdown(
                    f"#### **{rank}위. {row_data['선수명']}**"
                    f" ({row_data['시즌']})"
                )
                st.caption(
                    f"📌 포지션: `{row_data['포지션']}` | 리그:"
                    f" `{row_data['원소속리그']}`"
                )
                st.metric("매칭 유사도", f"{row_data['유사도(%)']}%")
                st.write(
                    f"- **실제 이적료**: €{row_data['실제이적료(만€)']:,.0f}만"
                    f" ({format_currency_desc(row_data['실제이적료(만€)']).split(' | ')[0]})"
                )
                st.write(
                    f"- **이적 총 평점**: ★ {row_data['이적평점']:.2f} / 10.00"
                )
                st.write(
                    f"- **평가율**: `{row_data['평가율(%)']:+.1f}%` (산출 적정가"
                    f" €{row_data['산출적정가(만€)']:,.1f}만)"
                )
                st.markdown("---")

        st.markdown(
            "#### 📋 **유사 이적 사례 전체 비교 테이블 (TOP 10 전체)**"
        )
        st.dataframe(
            match_df[[
                "유사도(%)",
        ...
        st.info(
            "💡 선택하신 포지션 또는 리그 필터 조건에 일치하는 과거 이적"
            " 데이터가 없습니다."
        )
    except Exception as e:
...
with tab4:
  st.subheader(
      "🎯 이적 첫 시즌 실제 성적 입력 & 모델 예측 정확도 사후 검증"
  )
  st.caption(
      "시즌 종료 후 선수가 실제로 기록한 최종 스탯(xG, xA 포함)을 입력하여 모델"
      " 예측치와의 오차율 및 적중률을 산출하고 [검증데이터] 시트에 업데이트합니다."
  )
  ...
  if val_df.empty or len(val_df) == 0 or "이적시즌" not in val_df.columns:
    st.info(
        "💡 **아직 [검증데이터] 시트에 저장된 데이터가 없습니다.**\n\n- 2번 탭에서"
        " 10대 핵심 리그 이적 선수를 저장하시면 이곳에 자동으로 나타납니다."
    )
  else:
    ...
    with v_top1:
      sel_val_season = st.selectbox(
          "이적 시즌 선택", available_seasons, key="val_sel_season"
      )
    ...
    completed_cnt = len(
        filtered_season_df[filtered_season_df["입력상태"] == "✅ 검증 완료"]
    )
    pending_cnt = total_in_season - completed_cnt
    progress_pct = (
        (completed_cnt / total_in_season * 100) if total_in_season > 0 else 0.0
    )

    with v_top2:
      st.info(
          f"📊 **`{sel_val_season}` 검증 진행도**: **총 {total_in_season}명 중"
          f" {completed_cnt}명 완료 / {pending_cnt}명 대기** (`{progress_pct:.0f}%`"
          " 달성)"
      )

    show_pending_only = st.checkbox(
        "⏳ 실제 성적 미입력(검증 대기) 선수만 모아보기",
        value=True if pending_cnt > 0 else False,
        key="filter_pending_only",
    )

    if show_pending_only:
      target_player_pool = filtered_season_df[
          filtered_season_df["입력상태"] == "⏳ 검증 대기"
      ]
    else:
      target_player_pool = filtered_season_df

    available_players = (
        list(target_player_pool["선수명"].dropna().unique())
        if "선수명" in target_player_pool.columns
        else []
    )
    ...
    if not available_players:
      if show_pending_only:
        st.success(
            "🎉 이번 시즌 모든 10대 리그 영입 선수의 실제 성적 입력 및 검증이"
            " 100% 완료되었습니다!"
        )
      else:
      ...
      target_row = target_player_pool[
          target_player_pool["선수명"] == sel_val_player
      ].iloc[-1]
      ...
      st.markdown("---")
      st.markdown(
          f"#### 2️⃣ **'{sel_val_player}'** 선수의 [모델 예측치] vs [시즌 실제"
          f" 기록 입력] ({curr_status})"
      )
      ...
      pm2.metric(
          "예측 득점 (xG)", f"{proj_g:.1f}골", delta=f"xG {proj_xg:.2f}"
      )
      pm3.metric(
          "예측 도움 (xA)", f"{proj_a:.1f}도움", delta=f"xA {proj_xa:.2f}"
      )
      ...
      st.markdown("<br>", unsafe_allow_html=True)
      st.markdown(
          "##### 📥 시즌 종료 후 실제 최종 기록 입력 (FotMob 기준)"
      )

      exist_act_mins = (
          int(target_row.get("실제출전시간", 0))
          if pd.notnull(target_row.get("실제출전시간"))
          and str(target_row.get("실제출전시간")).strip() not in ["", "nan"]
          else int(proj_m)
      )
      exist_act_goals = (
          int(target_row.get("실제득점", 0))
          if pd.notnull(target_row.get("실제득점"))
          and str(target_row.get("실제득점")).strip() not in ["", "nan"]
          else int(round(proj_g))
      )
      exist_act_xg = (
          float(target_row.get("실제xG", 0.0))
          if pd.notnull(target_row.get("실제xG"))
          and str(target_row.get("실제xG")).strip() not in ["", "nan"]
          else float(proj_xg)
      )
      exist_act_assists = (
          int(target_row.get("실제도움", 0))
          if pd.notnull(target_row.get("실제도움"))
          and str(target_row.get("실제도움")).strip() not in ["", "nan"]
          else int(round(proj_a))
      )
      exist_act_xa = (
          float(target_row.get("실제xA", 0.0))
          if pd.notnull(target_row.get("실제xA"))
          and str(target_row.get("실제xA")).strip() not in ["", "nan"]
          else float(proj_xa)
      )
      exist_act_rating = (
          float(target_row.get("실제평점", 0.0))
          if pd.notnull(target_row.get("실제평점"))
          and str(target_row.get("실제평점")).strip() not in ["", "nan"]
          else float(proj_r)
      )
      exist_act_notes = (
          str(target_row.get("검증메모", ""))
          if pd.notnull(target_row.get("검증메모"))
          else ""
      )

      in_ac1, in_ac2, in_ac3, in_ac4, in_ac5, in_ac6 = st.columns(6)
      with in_ac1:
        act_mins_val = st.number_input(
            "실제 출전 시간(분)",
            0,
            4500,
            value=exist_act_mins,
            step=90,
            key="val_act_mins",
        )
      with in_ac2:
        act_goals_val = st.number_input(
            "실제 득점(Goals)",
            0,
            60,
            value=exist_act_goals,
            step=1,
            key="val_act_goals",
        )
      with in_ac3:
        act_xg_val = st.number_input(
            "실제 기대득점(xG)",
            0.0,
            50.0,
            value=exist_act_xg,
      ...
      with in_ac4:
        act_assists_val = st.number_input(
            "실제 도움(Assists)",
            0,
            40,
            value=exist_act_assists,
            step=1,
            key="val_act_assists",
        )
      with in_ac5:
        act_xa_val = st.number_input(
            "실제 기대도움(xA)",
            0.0,
            30.0,
            value=exist_act_xa,
      ...
      with in_ac6:
        act_rating_val = st.number_input(
            "실제 FotMob 평균 평점",
            4.0,
            10.0,
            value=exist_act_rating,
            step=0.01,
            key="val_act_rating",
        )

      act_notes_val = st.text_input(
          "사후 검증 스카우팅 총평 / 비고",
          value=exist_act_notes,
          placeholder=(
              "예: 리그 적응 성공, 모델 예측 xG 및 평점 정확도 매우 우수"
          ),
          key="val_act_notes",
      )
      ...
      st.markdown("---")
      st.markdown(
          "#### 3️⃣ **모델 예측 vs 실제 성적 1:1 정밀 대칭 비교 리포트**"
      )
      ...
      comp_col1.metric(
          "평점 적중률",
          f"{rating_accuracy}%",
          delta=f"{act_rating_val - proj_r:+.2f}점 오차",
      )
      comp_col2.metric(
          "실제 출전시간",
          f"{act_mins_val:,}분",
          delta=f"{mins_diff:+,.0f}분 차이",
      )
      comp_col3.metric(
          "실제 득점 (xG)",
          f"{act_goals_val}골",
          delta=f"xG 오차 {xg_diff:+.2f}",
      )
      comp_col4.metric(
          "실제 도움 (xA)",
          f"{act_assists_val}도움",
          delta=f"xA 오차 {xa_diff:+.2f}",
      )
      comp_col5.metric(
          "실제 공격포인트",
          f"{act_goals_val + act_assists_val}P",
          delta=(
              f"{(act_goals_val + act_assists_val) - (proj_g + proj_a):+.1f}P"
              " 차이"
          ),
      )

      if st.button(
          "🚀 '검증데이터' 시트에 실제 최종 기록 업데이트하기",
          type="primary",
          use_container_width=True,
          key="update_actual_btn",
      ):
        with st.spinner("구글 시트에 최종 실제 성적을 업데이트 중입니다..."):
          ...
          try:
            ...
            if res_json.get("status") == "success":
              st.success(
                  f"✅ '{sel_val_player}' 선수의 실제 최종 성적이"
                  " 성공적으로 기록되었습니다!"
              )
              st.cache_data.clear()
              st.rerun()
...
with tab5:
  st.subheader(
      "👥 신규 이적생 vs 과거 유사 이적 선수 다각도 벤치마크 (Multi-Comps)"
  )
  st.caption(
      "새로운 시즌 영입 선수의 프로필(나이, 포지션, 이적료 규모, 생산력)을 과거"
      " 시트에 누적된 다른 선수들의 실제 사례와 1:1 및 다차원으로 정밀"
      " 비교합니다."
  )
  ...
  if (
    ...
    st.info(
        "💡 **아직 과거 누적 데이터가 없습니다.**\n\n1번 및 2번 탭에서 선수"
        " 데이터를 2명 이상 저장하시면 과거 선수들과의 1:1 교차 비교 및"
        " 벤치마크 매칭이 활성화됩니다."
    )
  else:
    st.markdown(
        "#### 1️⃣ 신규 분석 대상 선수 프로필 설정 (1번 탭 데이터"
        " 자동 연동)"
    )

    p_curr_name = (
        player_name.strip() if player_name.strip() else "신규 영입 대상 선수"
    )
    ...
    p_curr_p90 = (st.session_state["f_xg"] + st.session_state["f_xa"]) / (
        (st.session_state["f_mins"] / 90.0)
        if st.session_state["f_mins"] > 0
        else 1.0
    )
    ...
    c_prof2.metric(
        "포지션 & 리그", f"{p_curr_pos}", f"{selling_league.split(' ')[1]}"
    )
    c_prof3.metric(
        "실제 거래액", f"€{p_curr_fee:,.0f}만", f"평점 ★{p_curr_score:.2f}"
    )
    c_prof4.metric(
        "90분당 xG+xA / 평점", f"{p_curr_p90:.2f}", f"FotMob ★{p_curr_rating:.2f}"
    )

    st.markdown("---")
    st.markdown(
        "#### 2️⃣ 과거 유사 프로필 선수 1:1 직접 선택 대조 (Head-to-Head)"
    )

    past_player_names = [
        str(n).strip()
        for n in history_df["선수명"].dropna().unique()
    ...
    if not past_player_names:
      ...
      past_target = history_df[
          history_df["선수명"] == selected_past_player
      ].iloc[-1]

      t_name = str(past_target.get("선수명", "선수"))
      t_season = str(past_target.get("이적시즌", "26/27"))
      t_age = (
          int(past_target.get("나이", 25))
          if pd.notnull(past_target.get("나이"))
          else 25
      )
      ...
      t_xg = (
          float(past_target.get("직전_xG", 0.0))
          if pd.notnull(past_target.get("직전_xG"))
          else 0.0
      )
      t_xa = (
          float(past_target.get("직전_xA", 0.0))
          if pd.notnull(past_target.get("직전_xA"))
          else 0.0
      )
      t_mins = (
          float(past_target.get("직전_출전시간", 2500))
          if pd.notnull(past_target.get("직전_출전시간"))
          else 2500.0
      )
      t_rating = (
          float(past_target.get("직전_평점", 7.0))
          if pd.notnull(past_target.get("직전_평점"))
          else 7.0
      )
      ...
      df_bench = pd.DataFrame({
          ...
          f"신규 대상: {p_curr_name}": [
              ...
              (
                  f"€{p_curr_fee:,.0f}만"
                  f" ({format_currency_desc(p_curr_fee).split(' | ')[0]})"
              ),
              f"€{fair_value:,.1f}만",
          ...
          f"과거 비교: {t_name} ({t_season})": [
              ...
              (
                  f"€{t_fee:,.0f}만"
                  f" ({format_currency_desc(t_fee).split(' | ')[0]})"
              ),
              f"€{t_fair:,.1f}만",
              f"★ {t_score:.2f} / 10.00",
              f"★ {t_rating:.2f}",
          ...
          "비교 격차 / 인사이트": [
              ...
              (
                  "동일 포지션"
                  if p_curr_pos in t_pos or t_pos in p_curr_pos
                  else "포지션 상이"
              ),
              (
                  "동일 리그 출신"
                  if selling_league.split(" ")[1] in t_league
                  else "리그 상이"
              ),
      ...
      p_val_scaled = [
          min(100, p_curr_fee / 1000 * 10),
          final_deal_score * 10,
          p_curr_rating * 10,
          min(100, p_curr_p90 * 100),
          (35 - p_curr_age) * 5,
      ]
      t_val_scaled = [
          min(100, t_fee / 1000 * 10),
          t_score * 10,
          t_rating * 10,
          min(100, t_p90 * 100),
          (35 - t_age) * 5,
      ...
      bench_fig.add_trace(
          go.Scatterpolar(
              r=p_val_scaled + [p_val_scaled[0]],
              theta=comp_categories + [comp_categories[0]],
              fill="toself",
              name=p_curr_name,
      ...
      bench_fig.add_trace(
          go.Scatterpolar(
              r=t_val_scaled + [t_val_scaled[0]],
              theta=comp_categories + [comp_categories[0]],
              fill="toself",
              name=f"{t_name} ({t_season})",
...
with tab6:
  st.subheader(
      "🏆 이적시장 구단별 종합 성적표 & 리그 파워 랭킹 & 데이터룸"
  )
  st.caption(
      "시트에 누적된 영입(IN) 및 방출(OUT) 데이터를 종합하여 순지출(Net"
      " Spend)과 '이적료 가중 평균 평점' 기반의 구단/리그별 순위를 산출하고,"
      " 오기입된 데이터를 관리 및 삭제합니다."
  )

  if (
      history_df.empty
      or len(history_df) == 0
      or "이적시즌" not in history_df.columns
  ):
    st.info(
        "💡 **아직 구글 시트에 누적된 이적 데이터가 없습니다.**\n\n1번 및"
        " 2번 탭에서 팀명을 포함하여 이적 데이터를 저장하시면 이곳에 구단별"
        " 성적표 및 리그별/전체 통합 파워 랭킹이 자동으로 집계됩니다."
    )
  else:
    rank_mode = st.radio(
        "분석 모드 선택",
        [
            "🏢 구단별 이적시장 종합 성적표 (Club Report Card)",
            "🌍 리그별 / 10대 리그 전체 통합 파워 랭킹 (Power Rankings)",
            "🛠️ 저장 데이터 조회 및 삭제 관리 (Data Management)",
        ],
        horizontal=True,
    )
    ...
    if "구단별" in rank_mode:
      st.markdown(
          "#### 🏢 **특정 구단의 이적시장 결산 성적표 (IN/OUT & 순지출)**"
      )

      c_rc1, c_rc2 = st.columns(2)
      all_seasons = list(history_df["이적시즌"].dropna().unique())
      with c_rc1:
        sel_season_club = st.selectbox(
            "조회할 이적 시즌",
            ["전체 시즌"] + all_seasons,
            index=0,
            key="report_season_sel",
        )

      club_filtered_df = (
          history_df
          if sel_season_club == "전체 시즌"
          else history_df[history_df["이적시즌"] == sel_season_club]
      )

      to_teams = (
          [
              str(t).strip()
              for t in club_filtered_df["이적팀명"].dropna().unique()
      ...
      from_teams = (
          [
              str(t).strip()
              for t in club_filtered_df["원소속팀명"].dropna().unique()
              if str(t).strip() not in ["", "nan"]
          ]
          if "원소속팀명" in club_filtered_df.columns
          else []
      )
      ...
      if not all_club_names:
        st.warning(
            "⚠️ 아직 시트에 구단명이 입력된 이적 데이터가 없습니다. 1번"
            " 탭에서 구단명을 입력하고 새로 저장해 보세요."
        )
      else:
        with c_rc2:
          sel_team_name = st.selectbox(
              "조회할 구단(팀) 선택", all_club_names, key="report_team_sel"
          )

        team_in_df = (
            club_filtered_df[
                club_filtered_df["이적팀명"].astype(str).str.strip()
                == sel_team_name
            ].copy()
            if "이적팀명" in club_filtered_df.columns
            else pd.DataFrame()
        ...
        team_out_df = (
            club_filtered_df[
                club_filtered_df["원소속팀명"].astype(str).str.strip()
                == sel_team_name
            ].copy()
            if "원소속팀명" in club_filtered_df.columns
            else pd.DataFrame()
        ...
        total_in_spent = (
            team_in_df["실제이적료(만€)"].astype(float).sum()
            if not team_in_df.empty and "실제이적료(만€)" in team_in_df.columns
            else 0.0
        )
        total_out_income = (
            team_out_df["실제이적료(만€)"].astype(float).sum()
            if not team_out_df.empty
            and "실제이적료(만€)" in team_out_df.columns
            else 0.0
        )
        ...
        if not team_in_df.empty:
          team_in_df["이적평점"] = team_in_df.apply(
              lambda r: get_adjusted_deal_score(r, is_buy_side=True), axis=1
          )
        if not team_out_df.empty:
          team_out_df["이적평점"] = team_out_df.apply(
              lambda r: get_adjusted_deal_score(r, is_buy_side=False), axis=1
          )

        all_team_trades = pd.concat([team_in_df, team_out_df])
        if (
            not all_team_trades.empty
            and "이적평점" in all_team_trades.columns
        ):
        ...
        if weighted_avg_score >= 8.5:
          club_grade = "💎 S등급 (이적시장 대성공)"
        elif weighted_avg_score >= 7.5:
          club_grade = "🌟 A등급 (매우 훌륭한 이적시장)"
        elif weighted_avg_score >= 6.8:
          club_grade = "⚖️ B등급 (준수한 실리 운영)"
        elif weighted_avg_score >= 6.0:
          club_grade = "⚠️ C등급 (다소 아쉬운 이적시장)"
        else:
          club_grade = "🚨 D등급 (패닉 / 재정 낭비)"

        st.markdown(
            f"### 🛡️ **'{sel_team_name}'** 이적시장 종합 성적표"
            f" ({sel_season_club})"
        )
        ...
        t_m1.metric(
            "총 영입 지출액 (IN)",
            f"€{total_in_spent:,.0f}만",
            f"{len(team_in_df)}명 영입",
        )
        t_m2.metric(
            "총 방출 수익 (OUT)",
            f"€{total_out_income:,.0f}만",
            f"{len(team_out_df)}명 방출",
        )
        t_m3.metric(
            "순지출 (Net Spend)",
            f"€{net_spend:+,.0f}만",
            delta=(
                f"{format_currency_desc(abs(net_spend)).split(' | ')[0]}"
                f" {'지출' if net_spend >= 0 else '수익'}"
            ),
            delta_color="inverse",
        )
        t_m4.metric(
            "이적시장 가중 평점",
            f"★ {weighted_avg_score:.2f} / 10.00",
            club_grade.split(" ")[0],
        )
        ...
        sub_tab1, sub_tab2 = st.tabs([
            f"🔵 영입 명단 ({len(team_in_df)}명)",
            f"🔴 방출 명단 ({len(team_out_df)}명)",
        ])
        ...
        with sub_tab1:
          if team_in_df.empty:
            ...
            avail_cols = [c for c in display_cols if c in team_in_df.columns]
            st.dataframe(team_in_df[avail_cols], use_container_width=True)
    ...
    # 2) 리그별 / 10대 리그 전체 통합 파워 랭킹 모드
      ...
      with c_rk1:
        sel_season_rk = st.selectbox(
            "조회할 이적 시즌",
            ["전체 시즌"] + all_seasons_rk,
            index=0,
            key="rk_season_sel",
        )
      ...
      league_filtered_df = (
          history_df
          if sel_season_rk == "전체 시즌"
          else history_df[history_df["이적시즌"] == sel_season_rk]
      )

      if "이적팀리그" in league_filtered_df.columns:
        auto_detected_leagues = [
            str(l).strip()
            for l in league_filtered_df["이적팀리그"].dropna().unique()
      ...
      if not auto_detected_leagues:
        st.warning(
            "⚠️ 아직 시트에 '이적팀리그'가 기록된 데이터가 없습니다. 1번"
            " 탭에서 이적팀 리그를 선택하고 새로 저장해 보세요."
        )
      else:
        league_options = [
            "🌐 [전체 10개 리그 통합 순위표 (All Leagues)]"
        ] + sorted(auto_detected_leagues)
        with c_rk2:
          sel_league_name = st.selectbox(
              "조회할 리그 범위 선택 (자동 생성 필터)",
              league_options,
              key="rk_league_sel",
          )
        ...
        l_target_df = (
            league_filtered_df
            if is_all_leagues
            else league_filtered_df[
                league_filtered_df["이적팀리그"] == sel_league_name
            ]
        )

        if "이적팀명" in l_target_df.columns:
          l_target_df = l_target_df[
              l_target_df["이적팀명"].notnull()
              & (l_target_df["이적팀명"].astype(str).str.strip() != "")
          ]
        ...
        if l_target_df.empty:
          ...
          unique_teams = sorted(
              list(l_target_df["이적팀명"].astype(str).str.strip().unique())
          )
          team_stat_rows = []
          ...
          def get_adj_score(r, is_buy):
            ...
            if is_buy and orig == "OUT":
              return round(max(1.0, min(10.0, 15.00 - rec)), 2)
            elif not is_buy and orig == "IN":
              return round(max(1.0, min(10.0, 15.00 - rec)), 2)
            return rec

          for t_name in unique_teams:
            in_trades = l_target_df[
                l_target_df["이적팀명"].astype(str).str.strip() == t_name
            ].copy()
            out_trades = (
                league_filtered_df[
                    league_filtered_df["원소속팀명"].astype(str).str.strip()
                    == t_name
                ].copy()
                if "원소속팀명" in league_filtered_df.columns
                else pd.DataFrame()
            )

            in_spent = (
                in_trades["실제이적료(만€)"].astype(float).sum()
                if not in_trades.empty and "실제이적료(만€)" in in_trades.columns
                else 0.0
            )
            out_income = (
                out_trades["실제이적료(만€)"].astype(float).sum()
                if not out_trades.empty
                and "실제이적료(만€)" in out_trades.columns
                else 0.0
            )
            net_val = in_spent - out_income

            if not in_trades.empty:
              in_trades["이적평점"] = in_trades.apply(
                  lambda r: get_adj_score(r, True), axis=1
              )
            if not out_trades.empty:
              out_trades["이적평점"] = out_trades.apply(
                  lambda r: get_adj_score(r, False), axis=1
              )

            all_trades = pd.concat([in_trades, out_trades])
            if (
                not all_trades.empty
                and "이적평점" in all_trades.columns
            ):
              fees = all_trades["실제이적료(만€)"].astype(float)
              scores = all_trades["이적평점"].astype(float)
              if fees.sum() > 0:
                weights = fees.apply(lambda x: max(x, 500.0))
                w_score = (scores * weights).sum() / weights.sum()
              else:
                w_score = scores.mean()
            else:
              w_score = 7.50
            ...
            t_league_label = (
                str(in_trades["이적팀리그"].iloc[0]).strip()
                if not in_trades.empty and "이적팀리그" in in_trades.columns
                else ""
            )
          ...
          ranked_df = pd.DataFrame(team_stat_rows).sort_values(
              by="가중이적평점", ascending=False
          ).reset_index(drop=True)
          ranked_df.index = ranked_df.index + 1
          ranked_df.index.name = "순위 (Rank)"

          title_prefix = (
              "유럽 전체 10개 리그 통합" if is_all_leagues else sel_league_name
          )
          st.markdown(
              f"### 🏆 **{title_prefix}** 구단 이적시장 파워 랭킹"
              f" ({sel_season_rk})"
          )
          ...
          if is_all_leagues and "이적팀리그" in l_target_df.columns:
            st.markdown("---")
            st.markdown(
                "##### 📊 **리그별 평균 이적시장 평점 및 총 거래액 비교**"
            )
            league_summary = (
                l_target_df.groupby("이적팀리그")
                .agg(
                    참여구단수=("이적팀명", "nunique"),
                    총거래건수=("선수명", "count"),
                    리그총거래액=("실제이적료(만€)", "sum"),
                ...
                .reset_index()
                .sort_values(by="리그평균평점", ascending=False)
                .reset_index(drop=True)
            )
            league_summary.index = league_summary.index + 1
            league_summary.index.name = "리그 순위"
            league_summary["리그평균평점"] = league_summary[
                "리그평균평점"
            ].round(2)
            st.dataframe(league_summary, use_container_width=True)

          best_deal_row = l_target_df.sort_values(
              by="이적평점", ascending=False
          ).iloc[0]
          worst_deal_row = l_target_df.sort_values(
              by="이적평점", ascending=True
          ).iloc[0]
    ...
    # 3) 저장 데이터 관리 및 삭제 모드
      ...
      st.markdown(
          "#### 🛠️ **구글 시트 저장 데이터 조회 및 삭제 관리 (Data"
          " Management)**"
      )
      st.caption(
          "테스트로 잘못 저장했거나 중복 저장된 선수 데이터를 선택하여 구글"
          " 시트에서 즉시 안전하게 삭제합니다."
      )
      ...
      with del_c1:
        sel_del_season = st.selectbox(
            "삭제 대상 이적 시즌 선택", del_seasons, key="del_season_sel"
        )

      del_season_df = history_df[history_df["이적시즌"] == sel_del_season]
      del_players = (
          list(del_season_df["선수명"].dropna().unique())
          if "선수명" in del_season_df.columns
          else []
      )

      with del_c2:
        sel_del_player = (
            st.selectbox("삭제할 선수 선택", del_players, key="del_player_sel")
            if del_players
            else None
        )

      if sel_del_player:
        target_del_row = del_season_df[
            del_season_df["선수명"] == sel_del_player
        ].iloc[-1]
        st.warning(
            f"⚠️ 삭제 대상: **'{sel_del_player}'** (시즌: `{sel_del_season}` |"
            f" 이적료: `€{target_del_row.get('실제이적료(만€)', 0):,.0f}만` | 평점:"
            f" `★{target_del_row.get('이적평점', 0):.2f}`)"
        )

        if st.button(
            f"🗑️ '{sel_del_player}' 데이터 구글 시트에서 영구 삭제하기",
            type="primary",
            use_container_width=True,
            key="del_exec_btn",
        ):
          with st.spinner("구글 시트에서 데이터를 삭제 중입니다..."):
            ...
            try:
              ...
              if res_json.get("status") == "success":
                st.success(
                    f"✅ '{sel_del_player}' 선수의 데이터가 성공적으로"
                    " 삭제되었습니다!"
                )
                st.cache_data.clear()
                st.rerun()
