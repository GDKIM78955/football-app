...
if "input_tm" not in st.session_state:
    st.session_state["input_tm"] = 4500
if "input_fee" not in st.session_state:
    st.session_state["input_fee"] = 0
if "input_wage" not in st.session_state:
...
try:
    ...
    st.error(f"⚠️ 탭 모듈을 불러오는 중 오류가 발생했습니다. `tabs/` 폴더 내 파일들을 확인해주세요. 상세: {e}")
    st.stop()
