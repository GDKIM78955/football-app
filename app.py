            ...
            with c_ld3:
                ...
                if st.button("📥 데이터 불러오기", type="primary", use_container_width=True):
                    if sel_e_player:
                        ...
                        st.session_state["f_xg"] = safe_float(get_col(row_raw, 18, 0.0))
                        st.session_state["f_assists"] = safe_int(get_col(row_raw, 19, 0))
                        st.session_state["f_xa"] = safe_float(get_col(row_raw, 20, 0.0))
                        ...
                        st.session_state["f_rating"] = safe_float(get_col(row_raw, 27, 6.5))

                        st.session_state["f_gk_saves"] = safe_int(get_col(row_raw, 42, 0))
                        st.session_state["f_gk_conceded"] = safe_int(get_col(row_raw, 43, 0))
                        st.session_state["f_gk_prevented"] = safe_float(get_col(row_raw, 44, 0.0))
                        ...
                        st.session_state["f_pass_pct"] = safe_float(get_col(row_raw, 50, 0.0))
                        st.session_state["f_duels_pct"] = safe_float(get_col(row_raw, 51, 0.0))
                        st.session_state["f_aerial_pct"] = safe_float(get_col(row_raw, 52, 0.0))
...
with tab4:
    ...
    if val_df.empty or len(val_df) == 0:
        ...
        def check_status(row):
            act_r = str(get_col(row, 28, "")).strip()
            act_m = str(get_col(row, 6, "")).strip()
